...
/**
 * Save a newly generated wallpaper to the database
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // 3. Insert wallpaper record using Neon
    try {
      ...
      // If table doesn't exist yet, that's ok - just log it
      if (insertError.code === '42P01') {
        console.log('⚠️ Wallpaper table does not exist yet. Please create it in Neon');
        return NextResponse.json({
          success: true,
