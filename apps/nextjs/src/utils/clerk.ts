...
export const isPublicRoute = createRouteMatcher([
  ...
  new RegExp("/(\\w{2}/)?libra-ai(.*)"), // Public Libra AI page
  new RegExp("/(\\w{2}/)?generator(.*)"), // Public generator page
...
export const middleware = clerkMiddleware(async (auth, req: NextRequest) => {
  ...
  if (isAuthPage) {
    if (isAuth) {
      return NextResponse.redirect(new URL(`/${locale}/generator`, req.url));
    }
    return null;
  }
  if (!isAuth) {
