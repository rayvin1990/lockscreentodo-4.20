...
import { ArrowRight, Bell, CheckCircle2, Circle, CloudOff, Database, ExternalLink, Globe, Image as ImageIcon, Languages, Layers, Palette, RefreshCw, Scale, Shield, Smartphone, Sparkles, UserX } from "lucide-react";
...
type Copy = {
  ...
  sectionWhenLockTodo: string;
  whenLockTodoItems: readonly string[];
  sectionWhenLST: string;
  whenLSTItems: readonly string[];
  sectionNotion: string;
  sectionNotionBody: string;
  notionButton: string;
  sectionBoth: string;
  sectionBothBody: string;
  faqTitle: string;
...
export function LockTodoAlternativeContent({ lang, copy, notionIntegrationUrl, locktodoSiteUrl }: Props) {
  const generatorHref = `/${lang}/generator`;
  const homeHref = `/${lang}`;
  const iconForDimension = (label: string) => {
    const l = label.toLowerCase();
    if (l.includes("source") || l.includes("来源")) return <Database className="h-4 w-4" />;
    ...
    if (l.includes("automation") || l.includes("自动化")) return <RefreshCw className="h-4 w-4" />;
    if (l.includes("storage") || l.includes("privacy") || l.includes("存储")) return <Shield className="h-4 w-4" />;
    if (l.includes("design") || l.includes("设计")) return <Palette className="h-4 w-4" />;
    if (l.includes("language") || l.includes("语言")) return <Languages className="h-4 w-4" />;
    if (l.includes("price") || l.includes("价格")) return <Circle className="h-4 w-4" />;
  ...
  return (
    <div className="min-h-screen bg-[#020205] text-white selection:bg-indigo-500/30">
      ...
      <main className="pt-24">
        ...
        <section className="px-6 py-20">
          <div className="container mx-auto max-w-4xl">
            <h2 className="text-3xl font-bold tracking-tight">{copy.sectionBoth}</h2>
            <p className="mt-4 max-w-2xl text-base leading-relaxed text-slate-400">{copy.sectionBothBody}</p>
          </div>
        </section>
