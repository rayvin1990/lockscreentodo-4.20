...
import { ArrowRight, Bell, CheckCircle2, Circle, CloudOff, ExternalLink, Globe, Image as ImageIcon, Layers, ListChecks, Palette, RefreshCw, Shield, Smartphone, Sparkles, UserX, Wrench } from "lucide-react";
...
type Copy = {
  ...
  sectionMethod1: string;
  sectionMethod1Body: string;
  sectionMethod1Link: string;
  sectionMethod2: string;
  sectionMethod2Body: string;
  sectionMethod3: string;
  sectionMethod3Body: string;
  sectionCompare: string;
  compareHeaders: readonly [string, string, string, string];
  compareRows: readonly (readonly [string, string, string, string])[];
  sectionWhich: string;
  whichItems: readonly string[];
...
export function AppleRemindersLockScreenContent({ lang, copy, notionIntegrationUrl, appleWidgetGuideUrl }: Props) {
  const generatorHref = `/${lang}/generator`;
  const homeHref = `/${lang}`;
  const iconForDimension = (label: string) => {
    const l = label.toLowerCase();
    if (l.includes("see") || l.includes("看到")) return <ListChecks className="h-4 w-4" />;
    if (l.includes("sync") || l.includes("同步")) return <RefreshCw className="h-4 w-4" />;
    if (l.includes("install") || l.includes("装 app")) return <Layers className="h-4 w-4" />;
    if (l.includes("platform") || l.includes("平台")) return <Smartphone className="h-4 w-4" />;
    if (l.includes("density") || l.includes("密度")) return <Circle className="h-4 w-4" />;
    if (l.includes("design") || l.includes("设计")) return <Palette className="h-4 w-4" />;
    if (l.includes("account") || l.includes("账号")) return <UserX className="h-4 w-4" />;
    if (l.includes("device") || l.includes("数据")) return <Shield className="h-4 w-4" />;
          ...
          <div className="container mx-auto max-w-5xl space-y-6">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-7">
              <div className="mb-4 flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold">{copy.sectionMethod1}</h2>
              </div>
              <p className="text-sm leading-relaxed text-slate-400">{copy.sectionMethod1Body}</p>
              <a
                href={appleWidgetGuideUrl}
                target="_blank"
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-7">
              <div className="mb-4 flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/[0.07]">
                  <CloudOff className="h-5 w-5 text-indigo-300" />
                </div>
                <h2 className="text-xl font-bold">{copy.sectionMethod2}</h2>
              </div>
              <p className="text-sm leading-relaxed text-slate-400">{copy.sectionMethod2Body}</p>
            </div>
            <div className="rounded-2xl border border-indigo-500/30 bg-indigo-500/[0.05] p-7">
              <div className="mb-4 flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-500/15">
                  <ImageIcon className="h-5 w-5 text-indigo-300" />
                </div>
                <h2 className="text-xl font-bold">{copy.sectionMethod3}</h2>
              </div>
              <p className="text-sm leading-relaxed text-slate-300">{copy.sectionMethod3Body}</p>
            </div>
          </div>
        </section>
              ...
              <table className="w-full min-w-[820px] border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-white/10 text-xs uppercase tracking-widest text-white/40">
                    ...
                    <th className="px-5 py-4 font-bold text-white/60">{copy.compareHeaders[1]}</th>
                    <th className="px-5 py-4 font-bold text-white/60">{copy.compareHeaders[2]}</th>
                    <th className="px-5 py-4 font-bold text-indigo-300">{copy.compareHeaders[3]}</th>
                  </tr>
                </thead>
                <tbody>
                  {copy.compareRows.map((row, index) => (
                    <tr key={row[0]} className={index % 2 === 0 ? "bg-white/[0.02]" : undefined}>
                      ...
                      <td className="px-5 py-4 align-top text-slate-400">{row[2]}</td>
                      <td className="px-5 py-4 align-top text-white">{row[3]}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <section className="px-6 py-20">
          <div className="container mx-auto max-w-4xl">
            ...
            <ul className="mt-8 space-y-3">
              {copy.whichItems.map((item, index) => (
